...
# 0000028 req-003: a marker-write failure is no longer fully silent. It now
# emits exactly one stderr line naming the hook and the write error, so a
# genuinely-down backend combined with an unwritable plan/ never produces
# zero signal. Assert the new behaviour instead of the old total silence.
# wc -l counts newlines, and $(...) strips the trailing one, so a single
# line with no trailing newline reports 0; count with awk instead.
