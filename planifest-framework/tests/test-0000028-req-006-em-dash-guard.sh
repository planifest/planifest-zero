...
mk_proj() {
  ...
  mkdir -p "$proj/plan/current" "$proj/plan/_archive" "$proj/plan/changelog" "$proj/docs" \
    "$proj/planifest-framework/skills" "$proj/planifest-framework/templates" \
    "$proj/planifest-framework/standards" "$proj/src"
  echo "$proj"
}
...
RESULT=$(run_guard_write "planifest-framework/standards/notes.md" "$WITH_SENTINEL")
RC=$(split_rc "$RESULT")
...
for prefix in "plan/current" "docs" "planifest-framework/skills" \
              "planifest-framework/templates" "planifest-framework/standards"; do
...
RESULT=$(run_guard_write "planifest-framework/hooks/enforcement/em-dash-guard.mjs" "$EM_DASH_LINE")
RC=$(split_rc "$RESULT")
