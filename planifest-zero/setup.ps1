...
function Invoke-PlanifestGuardrails {
    ...
    git config core.hooksPath planifest-zero/hooks
    Write-Host "  + git config core.hooksPath planifest-zero/hooks"
...
```
repo/
+-- planifest-zero/        <- The framework (skills, templates, schemas, standards)
...
if (-not $Tool) {
    ...
    Write-Host ""
    Write-Host "Usage: .\planifest-zero\setup.ps1 claude-code [flags]"
    Write-Host ""
    Write-Host "Tools:"
    ...
    Write-Host "Each tool's config: planifest-zero\setup\[tool].ps1"
    exit 0
}
...
Write-Host "  Source of truth: planifest-zero/"
