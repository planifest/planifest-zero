...
3. Fall back to `planifest-zero/standards/library-standards/{language}/prefer-avoid.md`
4. Also check `planifest-zero/standards/library-standards/databases/prefer-avoid.md` if a database client is being added
5. Cross-reference every dependency against the avoid list: substitute the preferred alternative for any match
6. Follow `planifest-zero/standards/library-standards/_version-policy.md` for version pinning
...
- If any file under `planifest-zero/` was modified during this P3 run, update `planifest-zero/component.yml` before committing:
...
- Include `planifest-zero/component.yml` in the P3 commit so the ship-agent reads the correct version when creating the git tag.
...
Follows the canonical Parallelism Directive (Parallelism Rules) in `planifest-zero/standards/agent-dispatch-standards.md`. Independent implementation work MUST be parallelised. Run this checklist **before writing any implementation code**:
