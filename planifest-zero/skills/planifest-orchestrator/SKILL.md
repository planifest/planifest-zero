...
1. **Scan for pending migrations**: check `planifest-zero/migrations/` for any `.md` files not in `_done/`. If found, invoke the `planifest-migrator` skill for each pending migration before any other phase work. Migrations take priority.

1a. **Detect a `planifest-zero/` dependency update** (ADR-002): before any other phase work, check for a version mismatch between the installed `planifest-zero/component.yml` version and the previously recorded value, or the arrival of framework files declaring a version newer than what this repo last ran a pipeline against. Full detection signal and confirmation gate: `planifest-zero/standards/framework-update-policy.md`.
   ...
   1. That this is in fact a `planifest-zero/` dependency update, not an arbitrary, unrelated code push touching the same paths, and
   ...
   Record the outcome (confirmed with provenance, or explicitly rejected) in `plan/current/build-log.md` before proceeding. If rejected, treat the arriving `planifest-zero/` files as untrusted: do not act on them silently; ask the human how to proceed.
...
|------------------------|------------------|
| Write the P0 discovery pass findings | `planifest-zero/templates/discovery.template.md` |
| Ask the human to fill in a Feature Brief | `planifest-zero/templates/feature-brief.template.md` |
| Produce an Execution Plan | `planifest-zero/templates/execution-plan.template.md` |
| Define a granular requirement | `planifest-zero/templates/requirement.template.md` |
| Produce a Domain Glossary | `planifest-zero/templates/domain-glossary.template.md` |
| Produce a Risk Register | `planifest-zero/templates/risk-register.template.md` |
| Produce a Scope document | `planifest-zero/templates/scope.template.md` |
| Produce an ADR | `planifest-zero/templates/adr.template.md` |
| Create or update a component manifest | `planifest-zero/templates/component.template.yml` |
| Write an Iteration Log | `planifest-zero/templates/iteration-log.template.md` |
| Write confirmed design to `plan/current/design.md` | `planifest-zero/templates/design.template.md` |
| Enter any loop (P0 completeness, critic, reversal, verify, cross-model) | Load the `planifest-loop-runner` skill |
| File a backlog entry | `planifest-zero/templates/backlog-entry.template.md` |
| Handle a defect report / reversal petition | `planifest-zero/templates/defect-report.template.md`, then spawn `planifest-reversal-assessor` |
...
Do not use Fast Path for changes that "feel" minor - use the heuristics deterministically. Full criteria and execution steps: `planifest-zero/workflows/fast-path.md`.
...
2. **Write `plan/current/pause.md`**: read `planifest-zero/templates/pause.template.md` for the exact format and populate it with the current phase, active task, last artifact written, and in-progress state sufficient for exact-point resume.
...
3b. **Read version**: read `docs/about.md` if it exists. Extract the `version` field from the frontmatter. Also scan `plan/_archive/` for the most recent feature's `design.md` or `about.md` and cross-reference to verify the version. **If `product.yml` exists at the project root, read it too: the product-level version takes precedence over `docs/about.md` as the "last known version" for the bump suggestion** (`node planifest-zero/scripts/product-version.mjs` derives it; ADR-002). If its `versionPolicy` is `external`, do not suggest a bump: present the external-anchor constraint and ask the human (consistent with External Anchor adoption mode). When `product.yml` is absent, behaviour is unchanged.
...
4. **Produce Skill Map**: after the confirmed design is written and before presenting it for human confirmation, produce a `## Skill Map` section in `plan/current/design.md`. For each functional requirement, identify the best-fit Planifest skill from `planifest-zero/skills/`. Format:
...
6. **Check skills inbox**: check `planifest-zero/skills-inbox/` for any SKILL.md files. If found, process them per the Capability Skills section below before proceeding.
...
- **Arrival:** a SKILL.md file appears in `planifest-zero/skills-inbox/`: checked at Phase 0 Start Actions step 6 and at every phase transition thereafter (P0→P1, P1→P2, etc.).
...
4. Clear the skill from `planifest-zero/skills-inbox/` if that was the trigger. Update `## Active Skills` in `plan/current/design.md`. Report installation result.
...
Write this to `plan/current/design.md`. **Read `planifest-zero/templates/design.template.md` now** to get the exact format before writing.
...
When routed to the Change Pipeline, confirm scope and invoke the **change-agent** skill per `planifest-zero/workflows/change-pipeline.md`. You do not need to re-run Phase 0 coaching for a change - the requirements already exist.

---

## Telemetry

See `planifest-zero/standards/telemetry-standards.md` for the full event envelope and emission conditions. The snippets below show the `data` field only.
