...
3. For each confirmed candidate, run:
   ```bash
   bash planifest-zero/scripts/promote-to-regression.sh \
...
1. Read `planifest-zero/templates/test-report.template.md`.
...
2. Read `planifest-zero/templates/about.template.md` for the exact format
...
1. **`product.yml` exists at the project root**: derive the version from it:
   ```bash
   node planifest-zero/scripts/product-version.mjs
   ```
...
2. **No `product.yml` and the project has exactly one component** (exit 4): read the `version` field from the single `component.yml` (for this repo: `planifest-zero/component.yml`). This is the unchanged pre-0000016 behaviour.
3. **No `product.yml` and the project has 2+ components**: create `product.yml` from `planifest-zero/templates/product.template.yml` with `versionPolicy: max-component-version`, populate `components[]` with each component's `{id, path}` (path to its `component.yml`, not its version), then derive as in case 1.
...
See `planifest-zero/standards/telemetry-standards.md` for the full event envelope and emission conditions.
