...
1. Check `{tool-dir}/.planifest-setup-flags` for the target tool. If it exists and is well-formed (see `planifest-zero/component.yml`), read `flags`, `backendUrl`, and report every flag at **high** confidence, source: marker file.
...
Run `bash planifest-zero/scripts/refresh-delete-boot-files.sh` (or `planifest-zero/scripts/refresh-delete-boot-files.ps1` on Windows) from the repo root. Do not delete files directly (e.g. with a freeform `rm` command) - always invoke this script.
