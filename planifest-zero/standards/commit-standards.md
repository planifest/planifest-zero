...
`planifest-zero/hooks/commit-msg` checks messages against these rules and **exits 1 (blocks the commit)** on any violation (ADR-008). Use `git commit --no-verify` to bypass intentionally.
