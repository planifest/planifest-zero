...
& .\planifest-zero\tests\test_setup.ps1
```
...
```bash
# From the repository root
./planifest-zero/tests/test_setup.sh
```

## How It Works

1. **Isolation**: The test script automatically generates a temporary directory (e.g., `planifest_test_XXXXXX`).
2. **Context Creation**: The entire `planifest-zero` source is copied into the temporary directory to simulate a real project environment.
...
& .\planifest-zero\setup.ps1 [tool-name]
