...
# Ensure we have a substantial number of assertions (at least 65;
# the five-phase cut retired some rail assertions and the telemetry
# removal (0000033) retired four more)
if [ "$REGRESSION_COUNT" -ge 65 ]; then
  assert_equals "0" "0" "req-001: regression pack contains substantial assertions ($REGRESSION_COUNT)"
else
  assert_equals "65+" "$REGRESSION_COUNT" "req-001: assertion count minimum"
fi
