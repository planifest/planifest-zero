...
{
  const dir = makeWorkDir("comp-match", { sentinel: true, designPaths: ["planifest-zero/"] });
  const code = runGateWrite(dir, "planifest-zero/hooks/enforcement/gate-write.mjs");
  assert_equals(0, code, "planifest-zero/ path permitted when in Component Paths");
}

console.log("\n--- Component path: non-matching path is blocked ---");

{
  const dir = makeWorkDir("comp-no-match", { sentinel: true, designPaths: ["planifest-zero/"] });
