...
- `docs/decisions-index.md` gains a row for the new ADR, and marks the superseded ADR-002
  row as superseded, naming the new ADR.

## Acceptance Criteria
- [ ] A new ADR file exists under `plan/current/adr/`, following
  `planifest-framework/templates/adr.template.md`.
- [ ] The ADR's content names `0000025 ADR-002` as the decision it supersedes.
- [ ] The ADR's content names `plan/state/{tool}.md` as the new location and
  `planifest-overrides/setup-config/{tool}.md` as the old one.
- [ ] The ADR's content states that the marker file's role, format, and location are
  unchanged.
- [ ] `docs/decisions-index.md` contains a row for the new ADR.
- [ ] `docs/decisions-index.md`'s row for 0000025 ADR-002 (or the row nearest it recording
  that decision) is marked "Superseded" and names the new ADR.
