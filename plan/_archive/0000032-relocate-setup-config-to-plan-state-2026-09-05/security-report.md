...
|----|----------|----------|----------|-------------|----------------|--------|
| S-001 | low (resolved) | injection | `planifest-zero/skills/planifest-refresh-setup/SKILL.md` lines 38-43 | As found: Step 3 validated the record's JSON shape, the four field names, and the `tool` value only. It did not constrain `flags` to the two flags `setup.sh` accepts, nor `backendUrl` to the http(s) shape `setup.sh` line 1097 enforces. The record is git-tracked, so a hostile or careless commit could place arbitrary strings in the command the agent builds in sub-step 4 and later runs in Step 7. Human confirmation in Step 4 was the only barrier before the shell parsed those strings. ADR-002's own rationale says "Flags are injected into a command, so the input must be validated". | Add two validation rules to Step 3 sub-step 1: each `flags` entry must be one of `--structured-telemetry-mcp` or `--strict-orchestrator`, and `backendUrl` must be `null` or match `^https?://[A-Za-z0-9.-]+(:[0-9]+)?(/[A-Za-z0-9._/-]*)?$`. Treat any other value as invalid and fall back. Applied during P5: SKILL.md Step 3 carries both rules, req-004's Input Validation section and ADR-002 decision 2 record them, and `test-0000032-req-004-refresh-setup-reads-record-first.sh` asserts them (22 cases pass). | closed |
...
- Refresh-setup record: SKILL.md lines 38-43 require a fenced JSON block that parses, all four fields, and a matching `tool`. They also require every `flags` entry to belong to the allowed set and `backendUrl` to be `null` or match `setup.sh`'s URL pattern. req-004's Input Validation section and ADR-002 decision 2 carry the same two rules, so spec and implementation agree.
...
S-001 was raised at Medium during this phase and fixed before the gate closed. The code paths for writing and deleting are narrow, quoted, and guarded. The refresh-setup skill now validates both the shape and the values of the record before it builds a command, so a hostile commit no longer reaches the shell on human confirmation alone.

Top actions before production:
1. None outstanding for this feature. S-001 is closed.
