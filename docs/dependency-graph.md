...
**`hooks/telemetry/` imports from `hooks/enforcement/`, never the reverse.** The enforcement tree installs unconditionally. The telemetry tree installs only under `--structured-telemetry-mcp`. Any helper with an enforcement caller must therefore live in the always-present tree. See 0000028 ADR-002.
...
The graph previously carried three subgraphs that no longer exist:

- `context-mode-hooks`, holding `block-grep.mjs`, `block-bash.mjs`, and `block-webfetch.mjs`, plus their edge to an external context-mode MCP server.
- `setup-hook-integration`, holding `setup.sh`, `setup.ps1`, `skill-sync.sh`, and the per-tool marker file.
- The Tier 1 adapter path, which routed Cursor, Windsurf, and Cline installs through `hooks/adapters/`.
