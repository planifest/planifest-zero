...
|----|------|------|--------|--------|---------|------|
| `planifest-zero` | Planifest Framework | component-pack | developer-tooling | active | Core standards, skills, hooks, and Claude Code setup scripts that enforce the confirmed-design pipeline. | [component.yml](../planifest-zero/component.yml) |
...
| `setup-hook-integration` | Its `src/` tree documented `setup.sh` and `setup.ps1`, which live in `planifest-zero/` and survive under that component. |
